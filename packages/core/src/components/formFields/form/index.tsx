...
export default class FormField
  extends Field<FormFieldConfig, IFormField, Array<any>, FormState>
  implements IField<Array<any>>
{
  ...
  render = () => {
    const { value = [], formLayout, data, config } = this.props
    const { label, fields, primaryField, insertText, removeText, canInsert, canRemove, canSort, canCollapse } = config
                ...
                return (
                  <React.Fragment key={index}>
                    {this.renderItemComponent({
                      ...
                      onSort: canSort
                        ? async (sortType: 'up' | 'down') => await this.handleSort(index, sortType)
                        : undefined,
                      canCollapse,
                      children: (fields || []).map((formFieldConfig, fieldIndex) => {
                          ...
                          children: (
                            <FormField
                              ...
                              onValueSet={async (path, value, validation, options) =>
                                this.handleValueSet(index, fieldIndex, path, value, validation, options)
                              }
                              ...
                              onValueListAppend={async (path, value, validation, options) =>
                                this.handleValueListAppend(index, fieldIndex, path, value, validation, options)
                              }
                              onValueListSplice={async (path, _index, count, validation, options) =>
                                this.handleValueListSplice(index, fieldIndex, path, _index, count, validation, options)
                              }
                              onValueListSort={async (path, _index, sortType, validation, options) =>
                                await this.handleValueListSort(
                                  index,
                                  fieldIndex,
                                  path,
                                  _index,
                        ...
                        // 渲染表单项容器
                        return <div key={fieldIndex}>{this.renderItemFieldComponent(renderData)}</div>
                      })
                    })}
                  </React.Fragment>
                )
